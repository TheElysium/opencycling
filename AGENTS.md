...
- `plan/` — pure domain logic for multi-week training plans, no I/O. `types.rs` defines `TrainingPlan`, `PlanWeek`/`PlanDay` (the rendered grid), `PlanEntry`/`PlanEntryView`, `EntryContent` (a workout, a note, or both). `schedule.rs` builds the day grid (`build_weeks`) and validates plan writes (`validate_plan_write`, `overlaps`/`check_no_overlap` — no two active plans may cover the same dates). `entry.rs` normalizes/validates entry content and decides when swapping a day's workout should drop its linked session (`clears_session`).
...
- `db/` (`DbActorHandle` in `command.rs`) — wraps SQLite (`db/migrations.rs` for schema), stores sessions/samples, settings row, Strava auth, and training plans/entries (`db/plan_store.rs`, `training_plans` + `plan_entries` tables). `get_plan_weeks` composes a plan's rows with `plan::schedule::build_weeks()` at the Tauri-command layer in `lib.rs`, not inside the actor.
...
SvelteKit routes: `/` (home: today card + device connection), `/workouts`, `/workouts/detail`, `/plans` (create/archive/delete plans, current-week summary), `/plans/[id]` (full week grid for one plan), `/session`, `/history`, `/history/[id]`, `/settings`. Sidebar hidden on `/session`.

Shared state lives in `.svelte.ts` rune stores: `lib/ble.svelte.ts`, `lib/session.svelte.ts`, `lib/workout.svelte.ts`, `lib/aero.svelte.ts`, `lib/library.svelte.ts` (singleton workout-library + FTP state — load + scan once, shared by the home page, `/plans`, and workout pickers). Helpers: `lib/db.ts`, `lib/settings.ts`, `lib/format.ts`, `lib/metrics.ts`, `lib/ftp.ts`, `lib/audio.ts`, `lib/devices.ts` (auto-connect matching), `lib/chart-scale.ts`, `lib/session-visuals.ts`, `lib/strava.ts` (thin wrappers over generated commands), `lib/updater.ts` (`@tauri-apps/plugin-updater`, no-op under `tauri dev`), `lib/export.ts` (save dialog, then delegates to the Rust `export_session_tcx` command), `lib/aero.ts` (pure, unit-tested webcam aero-position scoring; `lib/aero.svelte.ts` owns the MoveNet detector, bundled offline under `static/models/`). Plan pure helpers (all unit-tested, mirror the Rust `plan/` validation surface): `lib/plan-date.ts` (`todayMonday`, `planEndDate`, `currentPlan`), `lib/plan-entry.ts` (day-cell edit actions), `lib/plan-link.ts` (`shouldLinkPlanEntry` — link a finished session back to the day it was started from), `lib/plan-load.ts` (`weekLoad`, `entryIntensity` — per-week duration/TSS and per-entry intensity color), `lib/plan-start.ts` (`resolvePlanStart` — entry → workout + FTP to arm a session).

Reusable components in `lib/components/`: `WorkoutChart` / `WorkoutPreview` / `WorkoutThumb` / `WorkoutCard` (block bars), `ZoneBar`, `SessionChart`, session UI tiles (`MetricTile`, `MetricsStrip`, `PowerTile`, `CurrentBlockCard`, `SessionTimeline`, `SessionStatsPanel`, `SessionFinishedCard`, `SessionDetailRecap`, `BlocksList`, `FtpTestResult`), plan UI (`TodayCard` — home-page today's entry + quick start, `CurrentPlanWeek`, `PlanWeekRow`, `PlanDayCell`, `PlanNoteField`, `WeekLoadSummary`, `WorkoutPicker` — modal library search/browse), aero UI (`AeroCalibration`, `AeroPanel`), sim UI (`SimPanel`, hidden when sim mode is off).
