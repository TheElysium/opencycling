<script lang="ts">
  ...
  import { computeWorkoutMetrics, type WorkoutType } from '$lib/metrics';
...
<div>
  ...
  {:else}
    <div class="workout-grid">
      {#each filteredWorkouts as { w, cardFtp, flat, lastUsedLabel }, i (i)}
        <WorkoutCard workout={w} {flat} ftpWatts={cardFtp} {lastUsedLabel} onclick={() => select(w)} />
      {/each}
    </div>
  {/if}
</div>
