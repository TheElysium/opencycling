...
impl DbActor {
        ...
        loop {
            match self.cmd_rx.recv().await {
                ...
                Some(cmd) => match cmd {
                    ...
                    DbCommand::GetPlanEntry { id, reply } => {
                        let _ = reply.send(plan_store::get_entry(&self.conn, id));
                    }
                    DbCommand::ListPlanEntries { plan_id, reply } => {
                        let _ = reply.send(plan_store::list_entries(&self.conn, plan_id));
                    }
    ...
    fn finalize_session(
        ...
        let ftp_w_used: u16 = self.conn.query_row(
            "SELECT ftp_w_used FROM sessions WHERE id = ?1",
            [session_id],
            |row| row.get(0),
        )?;

        let (avg_power, max_power, avg_hr, max_hr, avg_cad, max_cad): SessionAggregates =
            self.conn.query_row(
                "SELECT \
        ...
        let aero_pct: Option<f64> = self.conn.query_row(
    ...
    fn get_session(&self, id: i64) -> Result<SessionDetail, AppError> {
        ...
        let (mut detail, flat_blocks_json) = self.conn.query_row(
        ...
        // serde_json error is not a rusqlite::Error; map to Other to preserve context.
        detail.flat_blocks = serde_json::from_str::<Vec<FlatBlock>>(&flat_blocks_json)
            .map_err(|e| AppError::Other(format!("flat_blocks JSON: {e}")))?;
    ...
    fn get_strava_auth(&self) -> Result<Option<StravaAuth>, AppError> {
        self.conn
            ...
            .or_else(|e| match e {
                ...
                other => Err(AppError::DbError(other)),
            })
    }
